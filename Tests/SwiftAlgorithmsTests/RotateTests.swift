...
final class RotateTests: XCTestCase {
  ...
  func testUnderscoreReverse() {
    ...
    let limit: Int = 4
    let (lower, upper) = input._reverse(
      subrange: input.startIndex..<input.endIndex,
      until: input.startIndex.advanced(by: limit))
    let expected = [
      "p", "o", "n", "m", "e", "f", "g", "h", "i", "j", "k", "l", "d", "c", "b",
      "a",
    ]
  ...
  func testRotate() {
    for length in 0...15 {
      ...
      for j in 0..<length {
        ...
        expectEqualSequences(a[j...] + a[..<j], b)
