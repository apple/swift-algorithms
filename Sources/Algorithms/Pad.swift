...
extension RangeReplaceableCollection {
  /// Returns a `RangeReplaceableCollection` whose length is expanded
  ...
  /// `String`s justified to desired `paddedCount`.
  ///
  ...
  ///
  /// - Complexity: O(_m_), when collection's count is less than the `paddedCount`,
  /// where _m_ is the `paddedCount`. O(_n_) when the collection's count is
  /// greater than or equal to the `paddedCount`, where _n_ is the `count`
  /// of the collection. For a `RandomAccessCollection` when the collection's
  /// count is greater than or equal to the `paddedCount`, the complexity is
  /// reduced to O(_1_).
  @inlinable
  ...
  /// justified to desired `paddedCount`.
  ///
