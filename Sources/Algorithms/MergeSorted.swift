...
extension RangeReplaceableCollection where Element: Comparable {
  ...
  /// Given two sorted sequences treated as sets, create a copy of
  /// the first sequence without the elements that have a match in
  /// the second sequence (*i.e.* set difference).
  ///
  /// - Precondition: Both `first` and `second` must be sorted, and both
  ///   must be finite.
  ///
  ...
  @inlinable
  public init<T: Sequence, U: Sequence>(
    sorted first: T, withoutElementsFrom second: U
  ) where T.Element == Element, U.Element == Element {
    self.init(mergeSorted: first, and: second, retaining: .firstWithoutSecond)
  }

  /// Given two sorted sequences treated as sets, create a sorted sequence
  /// composed of the unmatched elements from both arguments (*i.e.* symmetric
  /// set difference).
  ///
  /// - Precondition: Both `first` and `second` must be sorted, and both
  ///   must be finite.
  ...
  @inlinable
  public init<T: Sequence, U: Sequence>(
    exclusivesOfSorted first: T, andExclusivesOf second: U
  ) where T.Element == Element, U.Element == Element {
    self.init(mergeSorted: first, and: second, retaining: .symmetricDifference)
  }

  /// Given two sorted sequences treated as sets, create a sorted sequence
  /// composed of the elements that are present in both arguments, but
  /// only one element per matched pair (*i.e.* set intersection).
  ///
  /// - Precondition: Both `first` and `second` must be sorted, and both
  ///   must be finite.
  ///
  ...
  @inlinable
  public init<T: Sequence, U: Sequence>(
    elementsOfSorted first: T, sharedWith second: U
  ) where T.Element == Element, U.Element == Element {
    self.init(mergeSorted: first, and: second, retaining: .intersection)
  }

  /// Given two sorted sequences treated as sets, create a sorted sequence with
  /// all the elements from both arguments, except matching pairs of
  /// elements appear only once (*i.e.* set union).
  ///
  /// To retain both elements of a matched pair, use `.init(mergeSorted:and:)`.
  ///
  /// - Precondition: Both `first` and `second` must be sorted, and both
  ///   must be finite.
  ///
  ...
  @inlinable
  public init<T: Sequence, U: Sequence>(
    unionOfSorted first: T, and second: U
  ) where T.Element == Element, U.Element == Element {
    self.init(mergeSorted: first, and: second, retaining: .union)
  }
}
