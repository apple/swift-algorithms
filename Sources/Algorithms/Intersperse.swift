...
public struct Intersperse<Base: Sequence> {
  ...
  @usableFromInline internal let separator: Base.Element

  @inlinable internal init(base: Base, separator: Base.Element) {
    self.base = base
...
extension Intersperse: Sequence {
  ...
  public struct Iterator: IteratorProtocol {
    ...
    @usableFromInline internal var state = State.start

    @inlinable internal init(iterator: Base.Iterator, separator: Base.Element) {
  ...
  @inlinable public func makeIterator() -> Intersperse<Base>.Iterator {
...
extension Intersperse: Collection where Base: Collection {
  ...
  public struct Index: Comparable {
    @usableFromInline enum Representation: Equatable {
      case element(Base.Index)
      case separator(next: Base.Index)
    }

    @usableFromInline internal let representation: Representation

    @inlinable init(representation: Representation) { self.representation = representation }

    @inlinable public static func < (lhs: Index, rhs: Index) -> Bool {
      switch (lhs.representation, rhs.representation) {
      case let (.element(li), .element(ri)), let (.separator(next: li), .separator(next: ri)),
        let (.element(li), .separator(next: ri)):
        return li < ri
      case let (.separator(next: li), .element(ri)): return li <= ri
      }
    }

    @inlinable static func element(_ index: Base.Index) -> Self {
      Self(representation: .element(index))
    }

    @inlinable static func separator(next: Base.Index) -> Self {
  ...
  @inlinable public var endIndex: Index { .separator(next: base.endIndex) }

  @inlinable public func index(after i: Index) -> Index {
    precondition(i != endIndex, "Can't advance past endIndex")
    switch i.representation {
    case let .element(index): return .separator(next: base.index(after: index))
    case let .separator(next): return .element(next)
    }
  }
  ...
  @inlinable public func index(_ i: Index, offsetBy distance: Int) -> Index {
    ...
    case (let .element(index), true): return .element(base.index(index, offsetBy: distance / 2))
  ...
  @inlinable public func distance(from start: Index, to end: Index) -> Int {
    ...
    case let (.element(start), .element(end)), let (.separator(start), .separator(end)):
      return 2 * base.distance(from: start, to: end)
    }
  }
}

extension Intersperse: BidirectionalCollection where Base: BidirectionalCollection {
  @inlinable public func index(before i: Index) -> Index {
    precondition(i != startIndex, "Can't move before startIndex")
    switch i.representation {
    case let .element(index): return .separator(next: index)
    case let .separator(next): return .element(base.index(before: next))
    }
  }
}

extension Intersperse: RandomAccessCollection where Base: RandomAccessCollection {}

extension Intersperse: LazySequenceProtocol where Base: LazySequenceProtocol {}

extension Intersperse: LazyCollectionProtocol where Base: LazyCollectionProtocol {}
...
extension Sequence {
  ...
  ///
  /// - Complexity: O(1)
  @inlinable public func interspersed(with separator: Element) -> Intersperse<Self> {
